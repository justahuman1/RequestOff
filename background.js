  ...
  observe: ({ frameId, tabId, url }) => {
    ...
    if (frameId === 0) {
      const redirectUrl = chrome.runtime.getURL(
        `/data/offline/index.html?id=${tabId}&rd=${encodeURIComponent(
          url
        )}&mode=${tabs[tabId] ? "per-tab" : mode}`
      );
...
var reset = async (enabled) => {
  ...
  const title = enabled
    ? "Your browser is in offline mode"
    : `Your browser is in online mode
...
reset();
webext.storage
  .on("changed", (ps) => reset(ps.enabled.newValue))
  .if((ps) => ps.enabled);
...
chrome.contextMenus.create({
  ...
  contexts: ["PAGE", "TABS"]
    .map((key) => chrome.contextMenus.ContextType[key])
    .filter((s) => s),
});
webext.contextMenus
  .on("clicked", async (_, tab) => {
    if (tabs[tab.id]) {
      delete tabs[tab.id];
    } else {
      tabs[tab.id] = true;
...
//         const now = Date.now();
//         const doUpdate =
//           (now - prefs["last-update"]) / 1000 / 60 / 60 / 24 > 30;
//         webext.storage
//           .set({
//             version,
...
//                   version +
//                   "&type=" +
//                   (p ? "upgrade&p=" + prefs.version : "install"),
//                 active: p === false,
//               });
//             }
//           });
